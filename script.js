...
window.addEventListener("DOMContentLoaded", () => { const toggleBtn = document.getElementById("theme-toggle"); if (toggleBtn) { toggleBtn.onclick = () => { document.body.classList.toggle("dark"); toggleBtn.textContent = document.body.classList.contains("dark") ? "☀️" : "🌙"; }; }
...
} catch (err) { alert("❌ Server error while checking code. (Dev: use bypass code if set.)"); } }

function unlockTool(mode) { const genPanel = document.getElementById("generator-panel"); if (genPanel) genPanel.style.display = "block";

if (mode === "master") { const passToggleWrap = document.getElementById("password-toggle-container"); if (passToggleWrap) passToggleWrap.style.display = "block"; } }
...
const countEl = document.getElementById("possibility-count"); if (countEl) countEl.textContent = Possible: ${total.toLocaleString()} variations;

if (total > 50000) { const warn = document.getElementById("crash-warning-modal"); if (warn) warn.style.display = "flex"; }

const emails = new Set(); for (let mask = 1; mask < total; mask++) { let result = ""; for (let i = 0; i < username.length; i++) { result += username[i]; if (i < username.length - 1 && (mask & (1 << (username.length - 2 - i)))) { result += "."; } } emails.add(result + "@gmail.com"); }

const { cleaned, stats } = dedupeEmails(Array.from(emails)); latestVariations = cleaned; const passwords = enablePasswords ? generatePasswordsForEmails(latestVariations) : {};

const listEl = document.getElementById("variation-list"); if (listEl) { listEl.innerHTML = <p>Showing first 300 of ${latestVariations.length} variations (removed ${stats.duplicates} duplicates).</p> <ul>${latestVariations.slice(0, 300).map(e =><li>${e}${enablePasswords ? " | Pass: " + passwords[e] : ""}</li>).join("")}</ul>; } }
...
const lines = input.split(/\r?\n/).filter(x => x.includes("@")); const { cleaned, stats } = dedupeEmails(lines);

const blob = new Blob([cleaned.join("\n")], { type: "text/csv" }); triggerDownload(blob, "converted_deduplicated.csv");
...
const groups = groupEmailsByCanonical(lines); const dupGroups = Object.entries(groups).filter(([_, arr]) => arr.length > 1);

const dupResult = document.getElementById("dup-result"); if (!dupResult) return;

if (!dupGroups.length) { dupResult.innerHTML = <p>No duplicates found.</p>; return; }

const markup = dupGroups.map(([canon, arr]) => <li><strong>${canon}</strong>: ${arr.join(", ")}</li> ).join("");

dupResult.innerHTML = <p>Found ${dupGroups.reduce((n, [_, arr]) => n + (arr.length - 1), 0)} duplicates (Gmail rules).</p> <ul>${markup}</ul>; }
...
window.submitAccessCode = submitAccessCode; window.generateEmails = generateEmails; window.copyEmails = copyEmails; window.downloadEmails = downloadEmails; window.convertToCSV = convertToCSV; window.checkForDuplicates = checkForDuplicates; window.dismissCrashWarning = dismissCrashWarning;
